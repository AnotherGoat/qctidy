...
echo -e "\n\033[33m[1/4] Saving system information...\033[0m"
cargo run -p benchmarks --bin sysinfo

echo -e "\n\033[33m[2/4] Running cargo bench...\033[0m"
cargo bench

echo -e "\n\033[33m[3/4] Generating unified JSON (rust_benchmark_results.json)...\033[0m"
python3 scripts/export_rust_json.py

if [ "$UPDATE_CHARTS" = true ]; then
    echo -e "\n\033[33m[4/4] Updating charts and cross-language speedup table...\033[0m"
    python3 scripts/plot_rust_results.py
    python3 scripts/generate_speedup_table.py
else
    echo -e "\n\033[90m[4/4] Charts skipped (use --update-charts to regenerate).\033[0m"
fi
