...
This directory contains the full performance benchmark ecosystem designed to stress the Rust quantum circuit optimizer core (`qctidy`) and compare it with the original Python implementation.

## Directory Structure

The benchmark ecosystem is divided into the following modular subdirectories:

- **`benches/`**: Contains the pure Rust source code (`performance.rs`) that defines and runs the benchmark algorithms using the statistically rigorous **`criterion`** library.
...
- **`results/`**: The automatically generated output directory. This is where the scripts write the final charts (`.png`), consolidated JSON (`rust_benchmark_results.json`), cross-implementation speedup tables (`.csv` and `.md`), and the machine specifications (`system_info.json`).
...
Criterion does not record the hardware or operating system it runs on, so benchmark numbers have no context on their own. To save a detailed dump of the machine (OS, kernel, hostname, architecture, CPU model and frequency, physical/logical cores, memory and swap) as JSON, run:
...
This only collects and writes `benchmarks/results/system_info.json`; it does not run the benchmarks.
...
1. **Saves** the machine specifications to `results/system_info.json`.
2. **Builds and runs** `cargo bench` internally.
3. **Extracts** the hidden results generated by Criterion (`target/criterion`) using the Python scripts described above.
4. **Consolidates** everything into a single structured results file compatible with the previous format.
5. (Optional) If you use the `-UpdateCharts` or `--update-charts` parameter, it immediately regenerates the bar charts and speedup tables in the `results/` directory.
